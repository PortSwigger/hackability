window.Inspector = function(){
  ...
  result, domObjects, callbacks = {}, historyPos = 0,
  types = ['array','NaN','null','function','object','string','boolean','symbol','undefined','number','window','function constructor','object constructor','document','dom node','x-domain window','java bridge','location','x-domain location','dom collection'].sort(),
  multiLineInput = document.createElement('textarea'), singleLineInput;
  ...
  function switchInput(obj, type, val) {
    var input, parent = obj.input.parentNode, output;
    input = type === 'text' ? singleLineInput : multiLineInput;
  ...
  function registerEvents(element, obj) {
    ...
    element.onpaste = function(e){
      ...
      if(/[;\n]/.test(val) || /(?:try|do|[)>])\s*[{]/.test(val)) {
         switchInput(obj, 'textarea', val);
      } else {
        switchInput(obj, 'text', val);
      }
    ...
    element.onkeydown = function(e){
      ...
      if(event.keyCode === 38 && (element.type === 'text'||(element.type === 'textarea' && event.altKey))) {
        getHistory('up');
        if(event.preventDefault) {
          event.preventDefault();
        } else {
      ...
      } else if(event.keyCode === 40 && (element.type === 'text'||(element.type === 'textarea' && event.altKey))) {
        getHistory('down');
        if(event.preventDefault) {
          event.preventDefault();
        } else {
      ...
      } else if(event.keyCode === 9 && element.type === 'textarea' && typeof element.selectionStart === 'number') {
        val=element.value;
        ...
        if(element.value.length) {
          if(!event.shiftKey && !event.ctrlKey && element.type === 'text') {
          ...
          if(element.type === 'textarea' && !event.ctrlKey && !event.shiftKey) {
              return;
          }
        }
        if(event.preventDefault) {
  ...
  function setDomObjects(obj) {
      ...
      registerEvents(multiLineInput, obj);
