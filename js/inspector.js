window.inspector = function(){
  ...
  function createEnumerator(obj, isRoot, name, path, parent, interestingProp) {
    ...
    li.enumerate = function(interestingOnly, filter, js) {
        var i, j, ul = document.createElement('ul'), li, div, propCheck = {}, props = [], checkProp = {}, regex, interestingProps = [], interestingPropsLookup = {}, found;
        ...
        if(interestingOnly) {
          for(i=0;i<knownWindowProps.length;i++) {
            for(j=0;j<props.length;j++) {
              if(knownWindowProps[i] === props[j]) {
                checkProp['_check_'+props[j]] = 1;
                interestingPropsLookup['_check_'+props[j]] = 1;
                break;
              }
            }
          }
