window.inspector = function(){
  ...
  function createEnumerator(obj, isRoot) {
    ...
    li.enumerate = function(){
        ...
        for(i=0;i<props.length;i++) {
          ...
          div.className = 'box';
          div.appendChild(document.createTextNode('name:'+props[i]));
          li.appendChild(div);
          try {
            li.appendChild(createEnumerator(this.object[props[i]]));
          } catch(e){}
          ul.appendChild(li);
  ...
  function createOutput(output, isHTML) {
    var div = document.createElement('div');
    if(isHTML) {
      div.innerHTML = output;
    } else {
      div.appendChild(createEnumerator(output,true));
    }
